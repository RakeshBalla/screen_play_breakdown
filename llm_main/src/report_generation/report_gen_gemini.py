...
pdf_file_path = "/home/ntlpt19/Downloads/Thella Kaagitham Full Script.pdf"
...
def chunk_documents(documents, pages_per_chunk=100):
...
genai.configure(api_key=api_key_ge)

# Load the model
# model = genai.GenerativeModel("gemini-2.0-flash")  # or "gemini-pro", etc.
model = genai.GenerativeModel("gemini-2.5-flash-preview-05-20")  # or "gemini-pro", etc.

def llm_response(prompt, sys_prompt="You are a helpful assistant"):
    chat = model.start_chat(history=[])
    response = chat.send_message(f"{sys_prompt}\n\n{prompt}")
    # print("formatted_res:", response.text)
...
def generate_summary(chunk_text, word_limit):
    ...
    sys_prompt = "You are a helpful assistant skilled in screenplay and narrative analysis."
    model_start = time.time()
    summary = llm_response(prompt, sys_prompt)
...
def generate_coverage_report(chunk_summaries, total_page_count):
    ...
    model_start = time.time()
    response = llm_response(prompt, sys_prompt)
    total_duration = time.time() - model_start
    print(f"[{datetime.now()}] Time taken ==> {total_duration:.2f} seconds.")
    return response
